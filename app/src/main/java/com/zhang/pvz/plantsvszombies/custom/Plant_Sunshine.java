...
public class Plant_Sunshine extends Sunshine {
    MainActivity activity_ = null;
    private Bitmap plant_sunshine_frame_ = null;
    ...
    public Plant_Sunshine(int x, int y, DoublePlayerModeAView activity) {
        super(x, y);

        ty = y;
        tx = x - 20;
        this.gradient_ = ty / tx;
    ...
    protected void loadImage() {
        this.plant_sunshine_frame_ = activity_.sunImageFactory_.getImage(MainActivity.SunImageFactory.Sun_Plant);
    }
    ...
    public void move() {
        ...
        if (dx < 0 || dy < 30) {
            if (!this.isCollected) {
                ...
                this.doublePlayerModeAView_.sP_sun += 25;
                this.doublePlayerModeAView_.plantSunAmount_ += 25;
    ...
    public void draw(Canvas canvas) {
        if (luodijuli_ < 13)
            canvas.drawBitmap(this.plant_sunshine_frame_, dx, dy, paint_);
        else if (!(dx < 0 || dy < 30))
            canvas.drawBitmap(this.plant_sunshine_frame_, dx, dy, paint_);
    }
}
